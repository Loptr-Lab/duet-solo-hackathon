...
function createDomainVerifier(collection) {
  async function start(did, domainInput) {
    ...
    const host = '_duet-verify';
    const id = crypto.createHash('sha256').update(`${did}:${domain}`).digest('hex');
    const expiresAt = Date.now() + CHALLENGE_TTL_MS;
    await collection.doc(`domain_${id}`).set({ did, domain, host, token, status: 'pending', createdAt: Date.now(), expiresAt });
    return { domain, recordType: 'TXT', host, value: token, expiresAt };
  }
  ...
  async function verify(did, domainInput) {
    ...
    const records = await dns.resolveTxt(`${challenge.host}.${domain}`);
    ...
    await ref.set({ status: 'verified', verifiedAt }, { merge: true });
    return { domain, status: 'verified', verifiedAt };
  }
